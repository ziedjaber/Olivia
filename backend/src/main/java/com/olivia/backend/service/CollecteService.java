...
public class CollecteService {
    ...
    private Collecte mapDocumentToCollecte(com.google.cloud.firestore.DocumentSnapshot d) {
        try {
            ...
            if (c != null) {
                ...
                c.setStartDate(safeGetDateString(c.getStartDate()));
                c.setEndDate(safeGetDateString(c.getEndDate()));
            }
            return c;
        } catch (Exception e) {
            log.error("Manual mapping failed for {}: {}", d.getId(), e.getMessage());
            return null;
        }
    }

    private String safeGetDateString(Object val) {
    ...
    private Map<String, Object> toMap(Collecte c) {
        ...
        m.put("aiJustification", c.getAiJustification());
