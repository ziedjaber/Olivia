...
public class AiController {
    ...
    public ResponseEntity<?> suggestLogistics(@RequestBody Map<String, String> request) {
        try {
            ...
            if (body.contains("leaked") || body.contains("API key")) {
                return ResponseEntity.status(403).body(Map.of("error", "Votre clé API Gemini a été bloquée par Google (leaked). Veuillez en générer une nouvelle sur https://aistudio.google.com/ et la mettre à jour dans application.properties."));
            }
            return ResponseEntity.status(403).body(Map.of("error", "Gemini API Forbidden: " + e.getMessage()));
        } catch (Exception e) {
            e.printStackTrace();
