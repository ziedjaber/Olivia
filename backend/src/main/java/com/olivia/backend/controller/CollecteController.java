...
public class CollecteController {
    ...
    public ResponseEntity<?> getAssignedCollectes() {
        try {
            return ResponseEntity.ok(collecteService.getCollectesByChef(currentUid()));
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error fetching assigned collectes: " + e.getMessage());
        }
    }
    ...
    @PutMapping("/{id}/mark-logistics-ready")
    @PreAuthorize("hasAnyAuthority('ROLE_DIRECTEUR', 'ROLE_RESPONSABLE_LOGISTIQUE')")
    public ResponseEntity<?> markLogisticsReady(@PathVariable String id, @RequestParam boolean ready) {
        try {
            collecteService.markLogisticsReady(id, ready);
            logAudit("LOGISTIQUE_PRÊTE", "Collecte", id, "Logistique marquée comme " + (ready ? "prête" : "non prête"));
            return ResponseEntity.ok("Logistics readiness updated.");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Failed to update logistics readiness: " + e.getMessage());
        }
    }

    @PutMapping("/{id}/mark-workers-ready")
    @PreAuthorize("hasAnyAuthority('ROLE_DIRECTEUR', 'ROLE_CHEF_EQUIPE_RECOLTE')")
    public ResponseEntity<?> markWorkersReady(@PathVariable String id, @RequestParam boolean ready) {
        try {
            collecteService.markWorkersReady(id, ready);
            logAudit("OUVRIERS_PRÊTS", "Collecte", id, "Main d'œuvre marquée comme " + (ready ? "prête" : "non prête"));
            return ResponseEntity.ok("Workers readiness updated.");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Failed to update workers readiness: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> inviteOuvrier(@PathVariable String id, @RequestBody Map<String, Object> body) {
        try {
            ...
            Participation participation = participationService.invite(
                    ...
                    String.valueOf(collecte.getStartDate()), String.valueOf(collecte.getEndDate()),
