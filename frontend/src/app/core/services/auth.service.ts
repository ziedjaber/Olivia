    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      if (savedUser) {
        ...
        cleanHttp.get<User>('http://localhost:8080/api/users/me', { headers }).subscribe({
          next: (refreshedUser) => {
            // Logic: If the backend returns 'USER' but we have a more specific role in cache,
            // it might be a Firestore quota fallback. We keep our cached role in that case.
            const useCachedRole = user.role && user.role !== 'USER' && refreshedUser.role === 'USER';
            const finalRole = useCachedRole ? user.role : refreshedUser.role;
            
            const updated = { ...user, ...refreshedUser, role: finalRole };
            localStorage.setItem('user', JSON.stringify(updated));
            this.currentUser.set(updated);
            console.log('[AuthService] Identity synced. Role used:', finalRole);
          },
          error: (err) => {
