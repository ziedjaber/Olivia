...
export class NotificationService {
  ...
  markBulkAsRead(ids: string[]) {
    this.http.put(`http://localhost:8080/api/notifications/bulk-read`, ids, { responseType: 'text' })
      .subscribe({
        next: () => this.refreshNotifications(),
        error: (err) => console.error("Failed to mark bulk notifications as read", err)
      });
  }

  deleteBulk(ids: string[]) {
    this.http.post(`http://localhost:8080/api/notifications/bulk-delete`, ids, { responseType: 'text' })
      .subscribe({
        next: () => this.refreshNotifications(),
        error: (err) => console.error("Failed to delete bulk notifications", err)
      });
  }
