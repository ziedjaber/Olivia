    ...
    <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <header class="mb-10 animate-in">
        ...
        <h1 class="text-4xl font-black text-on-surface tracking-tighter" style="font-family: Manrope, sans-serif;">
          Report <span class="text-error italic">Incident</span>
        </h1>
        <p class="text-on-surface-variant text-sm font-medium mt-1">Submit high-priority field alerts directly to the Command Center.</p>
      </header>
      ...
      <div class="glass-panel p-8 border-white bg-white/40 shadow-2xl animate-up relative overflow-hidden">
        ...
        <form (ngSubmit)="onSubmit()" class="space-y-8 relative z-10">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-3">
              <label class="text-[10px] font-black text-outline uppercase tracking-widest">Incident Category</label>
            ...
            <div class="space-y-3">
              <label class="text-[10px] font-black text-outline uppercase tracking-widest">Impact Severity</label>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-3">
              <label class="text-[10px] font-black text-outline uppercase tracking-widest">Affected Sector / Estate</label>
              <div class="relative group">
                ...
                <select [(ngModel)]="report.vergerId" name="verger" (change)="onVergerChange()" required
                        class="w-full bg-surface-container-low border border-outline-variant/10 rounded-2xl pl-12 pr-6 py-4 appearance-none outline-none focus:border-error/40 focus:bg-white text-sm font-bold text-on-surface transition-all shadow-inner">
                  <option value="">Select an assigned orchard...</option>
            ...
            <div class="space-y-3">
              <label class="text-[10px] font-black text-outline uppercase tracking-widest">Mission Coordinates</label>
              <div class="relative group">
          ...
          <div class="space-y-4">
            <div class="flex justify-between items-end">
               <label class="text-[10px] font-black text-outline uppercase tracking-widest">Visual Evidence (Optical Intelligence)</label>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
               ...
               <div *ngIf="report.imageUrls!.length < 5"
                  ...
                  <span class="text-[9px] font-black uppercase text-outline-variant group-hover:text-error transition-colors">Add Photo</span>
               </div>
            </div>
            ...
            <div *ngIf="report.imageUrls!.length === 0" class="text-center py-6 bg-surface-container-lowest rounded-3xl border border-outline-variant/5">
               <p class="text-[10px] font-bold text-outline-variant italic uppercase tracking-tighter">No evidence attached yet</p>
            </div>
          </div>

          <!-- DESCRIPTION -->
          <div class="space-y-4">
            <label class="text-[10px] font-black text-outline uppercase tracking-widest">Incident Intelligence (Details)</label>
            <div class="relative">
              ...
              <textarea [(ngModel)]="report.description" name="desc" rows="4" required
                      ...
                      placeholder="Specify exactly what happened and any immediate mitigation steps taken..."></textarea>
            </div>
          </div>
          ...
          <div class="pt-6">
            <button type="submit" [disabled]="loading"
              ...
              <span class="relative z-10 flex items-center justify-center gap-3">
                 ...
                 <span>{{ loading ? 'Transmitting Data...' : 'Transmit Emergency Alert' }}</span>
                 <span class="material-symbols-outlined text-[18px]">send</span>
              </span>
              <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
            </button>
            <p class="text-center text-[9px] font-bold text-outline uppercase tracking-widest mt-4 opacity-50">By transmitting, you authorize immediate command intervention.</p>
          </div>
        </form>
      </div>
    </div>
...
export class AlerteReportComponent implements OnInit {
  ...
  onSubmit() {
    if (!this.report.vergerId || !this.report.description) {
      this.toastService.show('Please complete all core mission data.', 'error');
      return;
    }

    if (!this.authService.isLoggedIn()) {
      this.toastService.show('Session expired. Please re-authenticate.', 'error');
    ...
    this.alerteService.reportAlerte(finalReport).subscribe({
      next: () => {
        this.toastService.show('Incident transmitted to Command Center.', 'success');
      ...
      error: (err) => {
        ...
        if (err.error) {
           try {
             ...
             this.toastService.show(`Relay Error: ${detail.message || 'Verification Failed'}`, 'error');
           } catch(e) {
             console.error('[DIAGNOSTIC] RAW ERROR BODY:', err.error);
             this.toastService.show('Transmission failure. Check relay integrity.', 'error');
           }
        } else {
           this.toastService.show('Network failure. Check server connectivity.', 'error');
        }
        this.cdr.markForCheck();
      }
    });
