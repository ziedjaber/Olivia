...
export class MissionAssignmentsComponent implements OnInit {
  ...
  finalizeProvisioning() {
     ...
     this.activeMission.aiJustification = this.aiSelectionJustification || undefined;
  ...
  async markReady(mission: Collecte) {
     ...
     if (isConfirmed) {
        ...
        this.collecteService.updateCollecte(mission.id, mission).subscribe({
           next: () => {
              this.dialogService.alert('Success', 'Mission for ' + mission.vergerName + ' marked ready for collection!', 'success');
              this.fetchData();
           },
           error: (err) => {
              console.error(err);
