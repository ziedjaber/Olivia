    ...
    <div class="p-8 min-h-screen vibrant-mesh animate-fade-in">
      ...
      <header class="mb-12">
        <h1 class="text-4xl font-black tracking-tight text-on-surface mb-2">
          Logistique <span class="text-gradient">Intelligence</span>
        </h1>
        <p class="text-on-surface-variant font-medium">Indicateurs opérationnels en temps réel et distribution des actifs.</p>
      </header>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <div class="glass-panel p-6 rounded-3xl group shadow-sm">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-outline">Actifs Totaux</span>
          </div>
          <h2 class="text-3xl font-black text-on-surface">{{ resources.length }}</h2>
          <p class="text-[10px] text-outline mt-2 font-bold uppercase tracking-wider">Répartis sur 6 catégories</p>
        </div>
        ...
        <div class="glass-panel p-6 rounded-3xl group shadow-sm">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-outline">Commandes Actives</span>
          </div>
          <h2 class="text-3xl font-black text-on-surface">{{ pendingOrdersCount }}</h2>
          <p class="text-[10px] text-outline mt-2 font-bold uppercase tracking-wider">En attente d'approbation</p>
        </div>
        ...
        <div class="glass-panel p-6 rounded-3xl group shadow-sm">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-outline">Utilisation</span>
          </div>
        ...
        <div class="glass-panel p-6 rounded-3xl group shadow-sm">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-outline">Stock Critique</span>
          </div>
          <h2 class="text-3xl font-black text-on-surface">{{ criticalStockCount }}</h2>
          <p class="text-[10px] text-error/70 mt-2 font-bold uppercase tracking-wider">Moins de 10% de capacité</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="glass-panel p-8 rounded-3xl shadow-lg border border-white/40">
           <h3 class="text-xl font-black mb-8 border-b border-outline-variant/10 pb-4">Distribution par Catégorie</h3>
           <div class="space-y-6">
              <div *ngFor="let cat of distribution" class="space-y-2">
                <div class="flex justify-between items-end">
                  ...
                  <span class="text-sm font-black text-outline">{{ cat.count }} articles</span>
                </div>
        ...
        <div class="glass-panel p-8 rounded-3xl shadow-lg border border-white/40">
           <h3 class="text-xl font-black mb-8 border-b border-outline-variant/10 pb-4">Pouls du Pipeline</h3>
           <div class="space-y-6">
             <div *ngFor="let order of recentOrders" class="flex gap-4 p-4 rounded-2xl bg-white/40 hover:bg-white/60 transition-colors border border-white/20 shadow-sm">
                ...
                <div class="flex-grow">
                  ...
                  <p class="text-[10px] text-outline font-bold uppercase">A réservé {{ order.resources.length }} types pour la Mission {{ order.collecteId.substring(0,8) }}</p>
                </div>
                <div class="text-right">
                  <span class="px-2 py-0.5 rounded-full text-[8px] font-black uppercase"
                        ...
                        }">{{ order.status === 'APPROVED' ? 'APPROUVÉ' : 'EN ATTENTE' }}</span>
                </div>
             </div>
             <div *ngIf="recentOrders.length === 0" class="text-center py-12 text-outline italic text-sm">Aucun mouvement récent dans le pipeline d'approvisionnement.</div>
           </div>
        </div>
      </div>
    </div>
