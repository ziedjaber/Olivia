                 ...
                 <div *ngFor="let col of myCollectes" class="group relative bg-stone-50/50 p-6 rounded-[2rem] border border-transparent hover:border-stone-200 transition-all hover:bg-white hover:shadow-lg">
                    <div class="flex flex-col md:flex-row gap-6">
                       ...
                       <div class="flex flex-row md:flex-col gap-2 justify-end">
                          ...
                          <button *ngIf="col.statut === 'en_cours'" (click)="endCollecte(col)" class="px-6 py-3 bg-white text-stone-900 border border-stone-200 text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-stone-50 transition-all active:scale-95">Terminer</button>
                       </div>
                    </div>
                 </div>
              </div>
                 ...
                 <table class="w-full text-left">
                   ...
                   <tbody class="divide-y divide-stone-100">
                     <tr *ngFor="let u of availableWorkers" class="hover:bg-stone-50 transition-all">
                        ...
                        <td class="px-6 py-4">
                           <div class="flex items-center gap-2">
                              <input type="number" [(ngModel)]="salaryInputs[u.id]" placeholder="Montant" class="w-16 px-2 py-1.5 text-[10px] font-bold bg-white border border-stone-200 rounded-lg outline-none focus:border-black transition-colors text-center shadow-inner">
