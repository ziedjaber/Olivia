                 ...
                 <table class="w-full text-left">
                   <thead>
                     <tr class="bg-stone-100/50">
                       ...
                       <th class="px-6 py-5 text-right text-[9px] font-black text-outline uppercase tracking-widest"></th>
                     </tr>
                   </thead>
                   <tbody class="divide-y divide-stone-100">
...
export class ChefEquipeDashboardComponent implements OnInit {
  ...
  calculateStats() {
    ...
    // Simplified: in a real app, count actual assigned workers across all missions
    this.crewCapacityPercent = target > 0 ? 65 : 0; 
  }
  ...
  inviteWorker(workerUid: string) {
    ...
    this.collecteService.inviteOuvrier(this.selectedCollecte.id, workerUid, salary).subscribe({
      next: () => {
         ...
         this.salaryInputs[workerUid] = 0;
      },
      error: () => this.dialogService.alert("Error", "Failed to invite worker.", "danger")
    });
  }
  ...
  payWorker(participationId: string) {
    this.participationService.payWorker(participationId).subscribe({
      ...
      error: () => this.dialogService.alert("Error", "Failed to mark salary as paid.", "danger")
    });
  }

  async removeWorker(participationId: string) {
    const isConfirmed = await this.dialogService.confirm("Remove Crew Member", "Confirm removal of worker from the mission roster?", "danger");
    if (isConfirmed) {
      this.participationService.removeWorker(participationId).subscribe({
        next: () => this.loadParticipations(this.selectedCollecte!.id!),
        error: () => this.dialogService.alert("Error", "Failed to remove worker.", "danger")
      });
    }
  }
