...
export class MaterialOrdersComponent implements OnInit {
  ...
  async submitOrder() {
    ...
    if (payload.resources.length === 0) {
      await this.dialogService.alert("Validation Error", "Selection list cannot be empty.", "warning");
      return;
    }
    ...
    this.http.post('http://localhost:8080/api/resource-orders', payload).subscribe({
      ...
      error: (err) => this.dialogService.alert("Error", "Order submission failed: " + err.message, "danger")
    });
  }
}
