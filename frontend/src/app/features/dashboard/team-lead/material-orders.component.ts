  ...
  template: `
    <div class="max-w-7xl mx-auto py-6">
      <header class="mb-12 flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h1 class="text-4xl font-extrabold text-on-surface font-headline tracking-tight mb-2">Mes commandes de matériel</h1>
          <p class="text-on-surface-variant">Gérer les commandes de matériel pour les missions en cours.</p>
        </div>
          ...
          <div *ngFor="let col of myCollectes" class="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/10 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
            <div>
              ...
              <div *ngIf="col.requiredResources && col.requiredResources.length > 0" class="space-y-1 mb-4 pt-4 border-t border-outline-variant/5">
                <p class="text-[10px] font-black text-outline uppercase tracking-widest mb-2">Liste d'approvisionnement du Directeur:</p>
           ...
           <table class="w-full text-left border-collapse">
              ...
              <tbody>
                ...
                <tr *ngIf="myOrders.length === 0">
                   <td colspan="5" class="p-10 text-center text-outline italic font-medium">Aucune commande de matériel passée pour le moment. Sélectionnez une mission ci-dessus pour commencer.</td>
                </tr>
              </tbody>
           </table>
...
export class MaterialOrdersComponent implements OnInit {
  ...
  async submitOrder() {
    ...
    if (payload.resources.length === 0) {
      await this.dialogService.alert("Validation Error", "La liste de sélection ne peut pas être vide.", "warning");
      return;
    }

    this.http.post('http://localhost:8080/api/resource-orders', payload).subscribe({
      next: () => {
        this.dialogService.alert("Success", "Order placed successfully.", "success");
        this.showOrderModal = false;
        this.loadOrders();
      },
      error: (err) => this.dialogService.alert("Error", "Échec de la soumission de la commande : " + err.message, "danger")
    });
  }
}
