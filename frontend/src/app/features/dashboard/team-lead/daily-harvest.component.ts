   ...
   template: `
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-10 animate-fade-in">
      ...
      <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-stone-100 pb-8">
        <div>
          <span class="text-[10px] font-black text-primary uppercase tracking-[0.4em] mb-2 block">Unite de commandement terrain</span>
          <h1 class="text-4xl font-black text-on-surface font-headline tracking-tighter">Operations <span class="text-primary italic">quotidiennes</span></h1>
          <p class="text-on-surface-variant font-medium mt-1 opacity-60">Gerez en temps reel la progression des arbres et les rapports journaliers de tonnage.</p>
        </div>
        ...
        <div class="flex gap-4">
           <div class="bg-white/80 backdrop-blur-md px-6 py-4 rounded-3xl border border-stone-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                <span class="text-[9px] font-black text-outline uppercase tracking-wider block">Secteurs actifs</span>
                <span class="text-lg font-black text-on-surface">{{ activeCollectes.length }} Campagnes</span>
              </div>
           </div>
        </div>
      </header>
      ...
      <div *ngIf="loading" class="py-20 flex flex-col items-center justify-center text-outline animate-pulse">
         ...
         <p class="text-[10px] font-black uppercase tracking-widest">Synchronisation de l'etat operationnel...</p>
      </div>
      ...
      <div *ngIf="!loading && activeCollectes.length === 0" class="py-32 text-center border-2 border-dashed border-stone-100 rounded-[3rem] bg-stone-50/30">
         ...
         <h4 class="text-xl font-black text-on-surface mb-2">Aucune recolte active</h4>
         <p class="text-sm text-outline font-medium max-w-xs mx-auto">Les operations terrain sont disponibles uniquement pour les campagnes "En cours".</p>
         <button routerLink="/dashboard" class="mt-8 px-8 py-3 bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl hover:opacity-90 transition-all">Aller a la gestion des affectations</button>
      </div>
                           ...
                           <div class="flex flex-col gap-2">
                              ...
                              <ng-container *ngIf="col.statut === 'en_cours'">
                                <button (click)="toggleMap(col)" class="w-full py-3 border border-white/20 hover:bg-white/10 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2">
                                   ...
                                   {{ activeMapCol === col.id ? 'Fermer la carte' : 'Ouvrir la carte de suivi' }}
                                </button>
                                <button (click)="openResultsLedger(col)" class="w-full py-4 bg-primary text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-lg">
                                   <span class="material-symbols-outlined text-lg">edit_document</span> Gerer les resultats quotidiens
                                </button>
                                <button (click)="openCrewLedger(col)" class="w-full mt-2 py-4 border border-white/20 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-white/10 transition-all flex items-center justify-center gap-2 shadow-lg">
                                   <span class="material-symbols-outlined text-lg">payments</span> Registre equipe / Paiement
                                </button>
          ...
          <header class="p-10 pb-6 border-b border-stone-50 shrink-0">
             <div class="flex justify-between items-start">
                <div>
                   ...
                   <p class="text-xs text-stone-400 font-medium mt-1">Mettez a jour les metriques de production pour chaque jour de campagne.</p>
                </div>
             ...
             <table class="w-full text-left">
                <thead>
                   <tr class="border-b border-stone-100">
                      <th class="py-4 text-[10px] font-black text-stone-400 uppercase tracking-widest font-headline">Jour de mission</th>
                   ...
                   <tr *ngFor="let day of missionDays" class="group hover:bg-stone-50/50 transition-colors">
                      ...
                      <td class="py-6 text-right">
                         <button (click)="saveDayResult(day)" [disabled]="loadingDay === day.date" 
                            ...
                            {{ loadingDay === day.date ? "Synchronisation..." : "Mettre a jour l'enregistrement" }}
                         </button>
                      </td>
                   </tr>
                </tbody>
       ...
       <div class="relative bg-white w-full max-w-4xl max-h-[85vh] rounded-[3rem] shadow-2xl flex flex-col border border-stone-100 animate-slide-up overflow-hidden">
          <header class="p-10 pb-6 border-b border-stone-50 shrink-0">
             <div class="flex justify-between items-start">
                <div>
                   ...
                   <h3 class="text-3xl font-black text-on-surface font-headline tracking-tighter">{{ selectedCrewCollecte.vergerName }} Registre d'equipe</h3>
          ...
          <div class="flex-grow overflow-y-auto p-10 pt-6 custom-scrollbar">
             ...
             <table *ngIf="!loadingCrew" class="w-full text-left">
                <thead>
                   <tr class="border-b border-stone-100">
                      <th class="py-4 text-[10px] font-black text-stone-400 uppercase tracking-widest font-headline">Ouvrier</th>
                   ...
                   <tr *ngFor="let p of crewParticipations" class="group">
                      ...
                      <td class="py-6 text-right">
                         ...
                         <span *ngIf="p.salaryPaid" class="flex items-center gap-1.5 px-4 py-2 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-xl ml-auto w-fit">
                            <span class="material-symbols-outlined text-sm font-black">check_circle</span> Paye
                         </span>
                      </td>
                   </tr>
                </tbody>
          ...
          <footer class="p-8 bg-stone-50/50 border-t border-stone-100 flex justify-end">
             <button (click)="showCrewModal = false" class="px-8 py-3 bg-white border border-stone-200 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-stone-50 shadow-sm transition-all font-headline">Terminer</button>
          </footer>
       </div>
    </div>
  `,
...
export class DailyHarvestComponent implements OnInit {
   ...
   loadVergers() {
      this.vergerService.getAssignedVergers().subscribe(data => {
         this.vergers = data || [];
   ...
   verifyDay(col: Collecte) {
      ...
      this.collecteService.verifyDay(col.id).subscribe({
         next: (updated) => {
            ...
            this.dialogService.alert("Jour valide", "Le protocole de commande est initialise pour la journee.", "success");
            this.cdr.detectChanges();
         },
         error: () => this.dialogService.alert("Erreur de protocole", "Echec de l'initialisation de la session quotidienne.", "danger")
      });
   }
   ...
   saveDayResult(day: any) {
      ...
      this.collecteService.addDailyProgress(this.selectedResultsCollecte.id, payload).subscribe({
         next: (updated) => {
            ...
            this.dialogService.alert("Enregistrement mis a jour", `Metriques du ${day.date} synchronisees avec succes.`, "success");
            this.cdr.detectChanges();
         },
         error: () => {
            this.loadingDay = null;
            this.dialogService.alert("Erreur de synchronisation", "Echec de transmission des donnees au registre central.", "danger");
         }
      });
   }
   ...
   openCrewLedger(col: Collecte) {
      ...
      this.collecteService.getParticipations(col.id).subscribe({
         ...
         error: () => {
            this.dialogService.alert("Erreur", "Impossible de recuperer le registre d'equipe.", "danger");
            this.loadingCrew = false;
         }
      });
   }
   ...
   async payWorker(p: Participation) {
      ...
      const confirm = await this.dialogService.confirm(
         ...
         `Autoriser le paiement numerique de ${p.dailySalary || 0} TND a ${p.ouvrierName} ?`,
         "warning"
      );
      ...
      if (confirm) {
         this.participationService.payWorker(p.id).subscribe({
            next: () => {
               this.dialogService.alert("Paiement reussi", `Paiement declenche pour ${p.ouvrierName}.`, "success");
               p.salaryPaid = true;
               this.cdr.detectChanges();
            },
            error: (err) => this.dialogService.alert("Echec du paiement", err.error || "Erreur de traitement du paiement.", "danger")
         });
      }
   }
   ...
   syncSalary(p: Participation) {
      ...
      this.participationService.updateSalary(p.id, p.dailySalary).subscribe({
         next: () => {
            this.dialogService.alert("Taux synchronise", `Remuneration de ${p.ouvrierName} mise a jour a ${p.dailySalary} TND.`, "success");
            this.cdr.detectChanges();
         },
         error: () => this.dialogService.alert("Erreur de synchronisation", "Echec de mise a jour du taux de remuneration.", "danger")
      });
   }
