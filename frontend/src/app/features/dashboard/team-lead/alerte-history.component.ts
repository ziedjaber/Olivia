              ...
              <tr *ngFor="let a of pagedAlerts" class="hover:bg-primary/5 transition-colors group">
                <td class="p-6">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <h4 class="text-sm font-black text-on-surface uppercase">{{ getTypeLabel(a.type) }}</h4>
        ...
        <div class="glass-panel w-full max-w-5xl max-h-[90vh] overflow-hidden border-white bg-white shadow-2xl animate-up flex flex-col md:flex-row divide-x divide-outline-variant/10">
           ...
           <div class="md:w-1/2 p-10 flex flex-col">
              <header class="mb-8">
                 ...
                 <h2 class="text-4xl font-black text-on-surface tracking-tighter" style="font-family: Manrope, sans-serif;">{{ getTypeLabel(selectedDetail.type) }} Report</h2>
...
export class AlerteHistoryComponent implements OnInit, OnDestroy {
  ...
  confirmDelete(a: Alerte) {
    if (confirm('Are you sure you want to retract this incident report?')) {
      if (!a.id) return;
