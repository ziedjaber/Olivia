...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 bg-surface min-h-screen">
      <!-- HEADER -->
      <header class="mb-12 flex flex-col md:flex-row justify-between items-center gap-6 animate-in">
        <div class="space-y-2 w-full md:w-auto">
          <div class="flex items-center gap-3 mb-2">
            <span class="w-12 h-1.5 bg-primary rounded-full"></span>
            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Intelligence Opérationnelle</p>
          </div>
          <h1 class="text-4xl font-black text-on-surface tracking-tighter leading-none" style="font-family: Manrope, sans-serif;">
            Centre d'Urgence <span class="text-primary italic">& Incidents</span>
          </h1>
          <p class="text-on-surface-variant font-medium text-xs max-w-lg">Signalez et gérez les alertes critiques de vos secteurs assignés.</p>
        </div>
        
        <div class="flex items-center gap-3 w-full md:w-auto justify-end">
          <button (click)="showReportForm = !showReportForm" 
                  class="flex-grow md:flex-none px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 border-2"
                  [ngClass]="showReportForm ? 'bg-white text-outline border-outline-variant/20' : 'bg-error text-white border-error shadow-error/20 hover:-translate-y-1'">
            <span class="material-symbols-outlined text-[18px]">{{ showReportForm ? 'close' : 'add_alert' }}</span>
            {{ showReportForm ? 'Fermer' : 'Signaler Incident' }}
          </button>
          <button (click)="loadAlerts()" class="w-14 h-14 rounded-2xl bg-white flex items-center justify-center text-outline hover:text-primary transition-all border border-outline-variant/10 shadow-sm relative">
            <span class="material-symbols-outlined" [class.animate-spin]="isRefreshing">refresh</span>
            <div class="absolute top-0 right-0 w-3 h-3 bg-emerald-500 rounded-full border-2 border-white" *ngIf="autoRefreshActive"></div>
          </button>
        </div>
      </header>
            ...
            <tbody class="divide-y divide-outline-variant/10">
              <tr *ngFor="let a of pagedAlerts" class="hover:bg-primary/5 transition-colors group">
                ...
                <td class="p-6 max-w-xs">
                  <p class="text-[10px] font-black text-primary uppercase tracking-widest mb-1">{{ a.vergerName || 'Secteur Inconnu' }}</p>
           ...
           <div class="md:w-1/2 p-10 flex flex-col">
              ...
              <div class="flex-grow space-y-8 overflow-y-auto custom-scrollbar pr-4">
                 ...
                 <section class="grid grid-cols-2 gap-6">
                    ...
                    <div class="bg-surface-container-low/50 p-5 rounded-3xl border border-outline-variant/5">
                       <label class="text-[9px] font-black text-outline/40 uppercase tracking-[0.2em] mb-2 block">Secteur / Verger</label>
                       <p class="text-xs font-black text-on-surface">{{ selectedDetail.vergerName || 'Non spécifié' }}</p>
                    </div>
                    <div class="bg-surface-container-low/50 p-5 rounded-3xl border border-outline-variant/5">
                       <label class="text-[9px] font-black text-outline/40 uppercase tracking-[0.2em] mb-2 block">Coordonnées</label>
...
export class AlerteHistoryComponent implements OnInit, OnDestroy {
  ...
  onVergerChange() {
    ...
    if (verger) {
      ...
      this.report.vergerName = verger.nom;
    }
  }
