           ...
           <div class="relative z-10 space-y-8">
              ...
              <div *ngIf="selectedMission" class="pt-2">
                 <button *ngIf="!selectedMission.workersReady" (click)="markWorkersReady()"
                         class="w-full py-4 rounded-2xl bg-primary text-on-primary font-black text-[10px] uppercase tracking-[0.25em] shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all">
                    ✓ Mark Workers as Ready
                 </button>
                 <div *ngIf="selectedMission.workersReady"
                      class="w-full py-4 rounded-2xl bg-green-500/20 border border-green-400/40 text-green-400 font-black text-[10px] uppercase tracking-[0.25em] text-center">
                    Workers Ready ✓
                 </div>
              </div>
           </div>
...
export class WorkerDirectoryComponent implements OnInit {
  ...
  async markWorkersReady() {
    ...
    const isConfirmed = await this.dialogService.confirm('Workers Ready Protocol', 'Mark workers as ready for this mission? The Director will be notified.', 'info');
    if (isConfirmed) {
      ...
      this.collecteService.updateCollecte(this.selectedMission.id, this.selectedMission).subscribe({
        next: () => {
          this.dialogService.alert('Success', 'Workers marked as ready for collection!', 'success');
          this.loadData();
        },
        error: (err) => {
          this.dialogService.alert('Error', 'Error marking workers as ready.', 'danger');
          console.error(err);
        }
      });
    }
