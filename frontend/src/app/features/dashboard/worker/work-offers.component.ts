  ...
  template: `
    <div class="max-w-7xl mx-auto py-10 px-4 space-y-10 animate-fade-in">
      ...
      <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          <span class="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2 block">Opportunites professionnelles</span>
          <h1 class="text-5xl font-extrabold text-on-surface font-headline tracking-tighter leading-none">Place du marche</h1>
          <p class="text-on-surface-variant font-medium mt-2 opacity-60 italic">Consultez et gerez vos invitations de travail pour la recolte.</p>
        </div>
        <div class="flex gap-4">
           <div class="bg-white/50 backdrop-blur-sm px-6 py-4 rounded-2xl border border-stone-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                <span class="text-[9px] font-black text-outline uppercase tracking-wider block">Invitations en attente</span>
                <span class="text-lg font-black text-on-surface">{{ pendingOffers.length }} Offres</span>
              </div>
           </div>
        </div>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div *ngFor="let offer of participations" 
          ...
          <div class="p-8 border-b border-stone-50 flex justify-between items-start bg-stone-50/30">
             <div class="flex items-center gap-4">
                ...
                <div>
                   ...
                   <span class="text-[10px] font-bold text-outline uppercase tracking-[0.2em] opacity-60">Invite par le chef {{ offer.invitedByName }}</span>
                </div>
             </div>
          ...
          <div class="p-8 grid grid-cols-2 gap-6 flex-grow">
             <div class="space-y-4">
                <div class="flex items-start gap-3">
                   ...
                   <div>
                      <span class="block text-xs font-black text-on-surface uppercase">{{ offer.collecteLocation || 'Terrain A12' }}</span>
                      <span class="text-[9px] font-bold text-outline uppercase tracking-tighter">Site de mission</span>
                   </div>
                </div>
                <div class="flex items-start gap-3">
                   <div class="w-8 h-8 rounded-xl bg-stone-100 flex items-center justify-center text-outline">
                   ...
                   <div>
                      ...
                      <span class="text-[9px] font-bold text-outline uppercase tracking-tighter">Calendrier de recolte</span>
                   </div>
                </div>
             </div>

             <div class="bg-black/5 rounded-[2rem] p-6 flex flex-col justify-center items-center text-center">
                <span class="text-[9px] font-black text-outline uppercase tracking-[0.2em] mb-2">Proposition de remuneration</span>
          ...
          <div *ngIf="offer.status === 'INVITED'" class="p-8 pt-0 grid grid-cols-2 gap-4">
             <button (click)="reject(offer)" class="py-4 bg-white border border-stone-200 text-on-surface text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-stone-50 transition-all active:scale-95 shadow-sm">Refuser l'offre</button>
             <button (click)="accept(offer)" class="py-4 bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl hover:opacity-90 transition-all active:scale-95">Accepter et rejoindre</button>
          </div>
          <div *ngIf="offer.status !== 'INVITED'" class="p-8 pt-0">
             <div class="w-full py-4 text-center text-outline italic text-xs font-medium bg-stone-50 rounded-2xl border border-stone-100">
                Vous avez deja {{ offer.status | lowercase }} cette mission.
             </div>
          </div>
        </div>
        ...
        <div *ngIf="participations.length === 0" class="col-span-full py-40 border-2 border-dashed border-stone-100 rounded-[4rem] text-center flex flex-col items-center gap-6">
           ...
           <div>
              <h4 class="text-lg font-black text-on-surface uppercase tracking-widest">Aucune invitation active</h4>
              <p class="text-xs font-medium text-outline opacity-60">Attendez qu'un chef d'equipe vous recrute pour les prochaines missions.</p>
           </div>
        </div>
      </div>
    </div>
