...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, DirectorLogisticsComponent, WeatherWidgetComponent],
  template: `
             ...
             <div *ngFor="let c of recentCollectes" class="flex items-center gap-4 p-5 bg-stone-50 rounded-2xl border border-stone-100 hover:border-primary/20 transition-colors group">
               ...
               <div class="flex-grow">
                 ...
                 <div class="flex items-center gap-2 mt-0.5">
                   <p class="text-[10px] text-outline font-black uppercase tracking-wider">{{ c.statut }} • Chef: {{ c.chefName }}</p>
                   <span *ngIf="c.logisticsReady" class="px-2 py-[2px] rounded-full bg-green-100 text-green-700 text-[8px] font-black uppercase tracking-widest flex items-center gap-1">
                     <span class="material-symbols-outlined text-[10px]">check_circle</span> Ready
                   </span>
                 </div>
               </div>
