  ...
  template: `
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-10 animate-fade-in font-headline">
      ...
      <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-stone-100 pb-8">
        <div>
          <span class="text-[10px] font-black text-[#3e5219] uppercase tracking-[0.4em] mb-2 block">Infrastructure Management</span>
          <h1 class="text-4xl font-black text-[#1e1c12] tracking-tighter">Unités de <span class="text-[#3e5219] italic">Trituration</span></h1>
          <p class="text-[#1e1c12]/60 font-medium mt-1 italic">Control and monitoring of industrial milling assets.</p>
        </div>
        ...
        <div class="space-y-6">
           ...
           <div class="bg-white rounded-[2.5rem] p-8 border border-stone-100 shadow-xl space-y-4">
              ...
              <div class="relative">
                 ...
                 <input [(ngModel)]="searchTerm" type="text" placeholder="Search by name or city..."
      ...
      <div *ngIf="showForm" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-[#f4edde] w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 space-y-8 border border-white/20 animate-slide-up">
          <header>
            ...
            <h3 class="text-3xl font-black text-[#1e1c12] tracking-tighter">{{ isEditing ? 'Update' : 'Initialize' }} Milling Unit</h3>
          </header>
          ...
          <form class="space-y-6" (ngSubmit)="saveCenter()">
            ...
            <div class="flex gap-4 pt-4">
              <button type="button" (click)="toggleForm()" class="flex-1 py-4 bg-white border border-stone-200 text-[#1e1c12] text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-stone-50 transition-all font-headline">Discard</button>
              <button type="submit" [disabled]="!currentCenter.name || !currentCenter.latitude"
                ...
                {{ isEditing ? 'Update Protocol' : 'Initialize Unit' }}
              </button>
            </div>
          </form>
        </div>
  ...
  saveCenter() {
    ...
    request.subscribe({
      next: () => {
        this.toggleForm();
        this.loadCenters();
        this.dialogService.alert("Success", "Milling unit protocol updated.", "success");
      },
      error: () => this.dialogService.alert("Error", "Failed to sync unit data.", "danger")
    });
  }

  async onDelete(id: string | undefined) {
    if (!id) return;
    const ok = await this.dialogService.confirm("Purge Unit", "Permanently remove this industrial asset?", "danger");
    if (ok) {
