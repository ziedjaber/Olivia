...
@Component({
  ...
  template: `
                  ...
                  <td class="px-8 py-6 text-right">
                    <div class="flex justify-end items-center gap-2">
                      ...
                      <button (click)="viewDetails(c)"
                              class="w-10 h-10 rounded-xl bg-amber-50 hover:bg-amber-100 transition-colors text-amber-700 flex items-center justify-center border border-amber-100 shadow-sm"
    ...
    <div *ngIf="showAssetsModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 backdrop-blur-xl p-4 animate-in fade-in">
     ...
     <!-- MISSION DETAILS MODAL (DIRECTOR SUMMARY) -->
     <div *ngIf="showDetailsModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 backdrop-blur-xl p-4 animate-in fade-in">
        <div class="bg-white w-full max-w-4xl max-h-[92vh] flex flex-col rounded-[3.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8">
            <!-- Header -->
            <div class="p-10 border-b border-stone-100 flex justify-between items-center bg-stone-50/30">
                <div class="flex items-center gap-6">
                    <div class="w-16 h-16 rounded-[1.5rem] bg-primary/10 text-primary flex items-center justify-center">
                        <span class="material-symbols-outlined text-3xl">summarize</span>
                    </div>
                    <div>
                        <span class="text-[10px] font-black text-primary uppercase tracking-[0.3em] block mb-1">Résumé Stratégique de Mission</span>
                        <h3 class="text-3xl font-black text-on-surface tracking-tighter" style="font-family: Manrope, sans-serif;">{{ selectedMissionForDetails?.description }}</h3>
                    </div>
                </div>
                <button (click)="showDetailsModal = false" class="w-12 h-12 rounded-full bg-stone-50 hover:bg-stone-100 transition-all flex items-center justify-center text-stone-400">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
            ...
            <div class="p-10 overflow-y-auto flex-grow space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-primary/[0.03] p-8 rounded-[2.5rem] border border-primary/10 flex flex-col items-center text-center">
                        <span class="material-symbols-outlined text-primary mb-2">scale</span>
                        <p class="text-[10px] font-black text-outline uppercase tracking-widest mb-1">Total Récolté</p>
                        <p class="text-4xl font-black text-primary tracking-tighter">{{ totalWeightHarvested | number:'1.2-2' }} <span class="text-sm">kg</span></p>
                    </div>
                    <div class="bg-stone-50 p-8 rounded-[2.5rem] border border-stone-100 flex flex-col items-center text-center">
                        <span class="material-symbols-outlined text-stone-400 mb-2">calendar_today</span>
                        <p class="text-[10px] font-black text-outline uppercase tracking-widest mb-1">Début</p>
                        <p class="text-xl font-black text-on-surface">{{ selectedMissionForDetails?.startDate | date:'dd MMM yyyy' }}</p>
                    </div>
                    <div class="bg-stone-50 p-8 rounded-[2.5rem] border border-stone-100 flex flex-col items-center text-center">
                        <span class="material-symbols-outlined text-stone-400 mb-2">event_available</span>
                        <p class="text-[10px] font-black text-outline uppercase tracking-widest mb-1">Fin Prévue</p>
                        <p class="text-xl font-black text-on-surface">{{ selectedMissionForDetails?.endDate | date:'dd MMM yyyy' }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- Management Team -->
                    <div class="space-y-4">
                        <h4 class="text-xs font-black text-outline uppercase tracking-widest border-b border-stone-100 pb-2">Chaîne de Commandement</h4>
                        <div class="space-y-3">
                            <!-- Oleiculteur -->
                            <div class="flex items-center gap-4 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
                                ...
                                <div>
                                    <p class="text-[9px] font-black text-outline uppercase tracking-widest">Oléiculteur (Propriétaire)</p>
                                    <p class="text-sm font-bold text-on-surface">{{ getVergerForMission(selectedMissionForDetails?.vergerId || '')?.responsableName || 'N/A' }}</p>
                                </div>
                            </div>
                            <!-- Team Lead -->
                            <div class="flex items-center gap-4 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
                                ...
                                <div>
                                    <p class="text-[9px] font-black text-outline uppercase tracking-widest">Chef d'Équipe</p>
                                    <p class="text-sm font-bold text-on-surface">{{ selectedMissionForDetails?.chefName }}</p>
                                </div>
                            </div>
                            <!-- Logistics -->
                            <div class="flex items-center gap-4 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
                                ...
                                <div>
                                    <p class="text-[9px] font-black text-outline uppercase tracking-widest">Responsable Logistique</p>
                                    <p class="text-sm font-bold text-on-surface">{{ selectedMissionForDetails?.logisticsName || 'Non assigné' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center border-b border-stone-100 pb-2">
                            <h4 class="text-xs font-black text-outline uppercase tracking-widest">Équipe de Terrain</h4>
                            <span class="text-[10px] font-black text-primary px-2 py-0.5 bg-primary/5 rounded-md">{{ missionWorkers.length }} Moissonneurs</span>
                        </div>
                        <div class="max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            <div *ngIf="isLoadingAssets" class="space-y-2">
                                <div *ngFor="let i of [1,2,3]" class="h-12 w-full bg-stone-50 rounded-xl animate-pulse"></div>
                            </div>
                            <div *ngIf="!isLoadingAssets && missionWorkers.length === 0" class="py-10 text-center text-outline text-xs italic">
                                Aucun ouvrier assigné.
                            </div>
                            <div class="space-y-2">
                                <div *ngFor="let w of missionWorkers" class="flex items-center gap-3 p-3 rounded-xl border border-stone-50 hover:bg-stone-50 transition-colors">
                                    <div class="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-[10px] font-black text-stone-500">
                                        {{ w.ouvrierName.charAt(0) }}
                                    </div>
                                    <span class="text-sm font-medium text-on-surface">{{ w.ouvrierName }}</span>
                                    <span class="ml-auto text-[9px] font-black text-outline uppercase tracking-tighter opacity-40">{{ w.status }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Footer -->
            <div class="p-10 border-t border-stone-100 bg-stone-50/30 flex justify-end">
                <button (click)="showDetailsModal = false" class="bg-primary text-on-primary px-12 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all">Fermer le Résumé</button>
            </div>
        </div>
     </div>
   `,
...
export class HarvestPlanningComponent implements OnInit {
  ...
  selectedMissionForDetails: Collecte | null = null;
  ...
  viewDetails(c: Collecte) {
    if (!c.id) return;
    this.selectedMissionForDetails = c;
    this.totalWeightHarvested = (c.dailyReports || []).reduce((acc, r) => acc + (r.weightKg || 0), 0);
    this.missionWorkers = [];
    this.isLoadingAssets = true;
