...
export class HarvestPlanningComponent implements OnInit {
  ...
  openIntelligence(vergerId: string) {
    ...
    if (!verger) {
       alert("Synchronizing orchard intelligence... please wait.");
       return;
    }
  ...
  submitCollecte() {
    ...
    request.subscribe({
       ...
       error: (err) => alert("Mission operation failed: " + (err.error || err.message))
    });
  }

  onDeleteMission(c: Collecte) {
    if (!c.id) return;
    if (confirm(`Authorize permanent deletion of mission: "${c.description}"?`)) {
      this.collecteService.deleteCollecte(c.id).subscribe({
        next: () => this.loadData(),
        error: (err) => alert("Purge failed: " + (err.error || err.message))
      });
    }
  }
