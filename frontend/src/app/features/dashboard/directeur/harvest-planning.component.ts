              ...
              <tr *ngFor="let c of pagedCollectes" class="hover:bg-primary/[0.02] transition-colors group">
                ...
                <td class="px-8 py-6 text-right">
                  <div class="flex justify-end items-center gap-2 flex-wrap">
                     ...
                     <span *ngIf="c.workersReady" class="px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-[8px] font-black uppercase tracking-widest flex items-center gap-1"><span class="material-symbols-outlined text-[10px]">group</span> Équipe</span>
                    ...
                    <button (click)="viewAssets(c)"
                            ...
                            title="Voir les Ressources Assignées (Logistique & Équipe)">
                      ...
                      <button *ngIf="c.statut === 'termine'" (click)="goToTrituration()"
                        ...
                        <span class="material-symbols-outlined text-sm">oil_barrel</span> Planifier Trituration
                      </button>
                  </div>
                </td>
              </tr>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                <div class="group">
                  ...
                  <select [(ngModel)]="newCollecte.logisticsUid" class="w-full bg-surface-container-low border border-outline-variant/10 focus:border-primary/40 focus:bg-white rounded-[1.5rem] px-6 py-5 text-sm font-bold text-on-surface outline-none transition-all cursor-pointer">
                    <option [value]="undefined">Assigner un Responsable Logistique</option>
        ...
        <div class="bg-surface w-full max-w-4xl max-h-[90vh] flex flex-col rounded-[3.5rem] shadow-2xl overflow-hidden border border-white/5 animate-up">
            ...
            <div class="p-10 overflow-y-auto flex-grow grid grid-cols-1 md:grid-cols-2 gap-8 bg-surface-container-lowest">
                ...
                <div class="space-y-6">
                   ...
                   <div *ngIf="isLoadingAssets" class="text-xs text-outline italic">Chargement des données de l'équipe...</div>
                   <div *ngIf="!isLoadingAssets && missionWorkers.length === 0" class="text-xs text-outline italic bg-surface-container-low p-6 rounded-3xl text-center">Aucune équipe assignée pour le moment.</div>
                   ...
                   <div class="space-y-3">
                      <div *ngFor="let w of missionWorkers" class="bg-white p-4 rounded-3xl border border-outline-variant/10 flex items-center gap-4 hover:shadow-md transition-shadow">
                         ...
                         <div class="text-right">
                            <p class="text-[10px] font-black tracking-widest text-primary">{{ w.dailySalary }} TND/jour</p>
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <div class="space-y-6 md:border-l md:border-outline-variant/10 md:pl-8">
                   ...
                   <div *ngIf="isLoadingAssets" class="text-xs text-outline italic">Chargement des données logistiques...</div>
                   <div *ngIf="!isLoadingAssets && missionLogistics.length === 0" class="text-xs text-outline italic bg-surface-container-low p-6 rounded-3xl text-center">Aucune logistique préparée pour le moment.</div>
                   ...
                   <div class="space-y-3">
                      <div *ngFor="let order of missionLogistics" class="bg-white p-5 rounded-3xl border border-outline-variant/10 flex flex-col gap-3 shadow-sm hover:shadow-md transition-shadow">
                         <div class="flex justify-between items-center">
                            ...
                            <span class="text-[10px] font-medium text-outline">Commande #{{ order.id?.substring(0,6) }}</span>
                         </div>
