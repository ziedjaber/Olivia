...
onStatutChange(verger: Verger, newStatut: string) {
  ...
  if (!id) return;
  if (isPlatformBrowser(this.platformId)) {
    const isConfirmed = await this.dialogService.confirm('Supprimer le verger', 'Voulez-vous vraiment supprimer ce verger ? Cette action est irréversible.', 'danger');
    if (isConfirmed) {
