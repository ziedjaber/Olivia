...
@Component({
  ...
  template: `
    <header class="fixed top-0 right-0 w-[calc(100%-16rem)] z-40 bg-background/70 backdrop-blur-xl shadow-sm h-20 flex justify-between items-center px-8 border-b border-outline-variant/10">
      ...
      <div class="flex items-center gap-4">
        <button class="p-3 transition-colors duration-200 hover:bg-surface-container-low rounded-full relative" (click)="toggleChat()">
          ...
          <span *ngIf="(totalUnreadCount$ | async) as unread" class="absolute top-1 right-1 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">
            {{ unread > 9 ? '9+' : unread }}
          </span>
        </button>
